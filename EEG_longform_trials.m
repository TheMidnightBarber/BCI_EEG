...
output = {'subject','gender','age','hand','group','session','block','hemisphere','trial','ERD'};
sides = {'L','R'};
        ...
        if ~skip
            ...
            for blockno = 1:size(out.left_blocks,3)
                ...
                for chunkno = 1:nchunks
                    ...
                    ERD_LBLH = [ERD_LBLH -log2(bandpower(active.LH,out.fs,[f1 f2]) / bandpower(baseline.LH,out.fs,[f1 f2]))];
                    ERD_LBRH = [ERD_LBRH -log2(bandpower(active.RH,out.fs,[f1 f2]) / bandpower(baseline.RH,out.fs,[f1 f2]))];
                ...
                for chunkno = 1:nchunks
                    ...
                    ERD_RBLH = [ERD_RBLH -log2(bandpower(active.LH,out.fs,[f1 f2]) / bandpower(baseline.LH,out.fs,[f1 f2]))];
                    ERD_RBRH = [ERD_RBRH -log2(bandpower(active.RH,out.fs,[f1 f2]) / bandpower(baseline.RH,out.fs,[f1 f2]))];
...
fname = ['/Volumes/HDD/data/BCI/work/alldat-' int2str(chunksize*1000) '-trials_' code '.csv'];

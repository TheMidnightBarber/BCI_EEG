...
chunksize = 0.5; % Must be divisible into 12s, maximum 2s

exclusions = {'BCI15-14',4};
output = {'subject','gender','age','hand','group','session','block','hemisphere','trial','time','betaPower'};
sides = {'L','R'};
    ...
    for sno = 1:10
        ...
        if ~skip
            ...
            for blockno = 1:size(out.left_blocks,3)
                ...
                nchunks = size(out.left_blocks,2) / (chunksize*out.fs);
                ERD_LBLH = [];
                ERD_LBRH = [];
                
                for chunkno = 1:nchunks
                    
                    openingsamp = 1; % 2 * out.fs;
                    ...
                    ERD_LBLH = [ERD_LBLH bandpower(active.LH,out.fs,[15 30])];
                    ERD_LBRH = [ERD_LBRH bandpower(active.RH,out.fs,[15 30])];

                end
            
                % Right blocks
            
                nchunks = size(out.right_blocks,2) / (chunksize*out.fs);
                ERD_RBLH = [];
                ERD_RBRH = [];
                ...
                for chunkno = 1:nchunks
                    ...
                    ERD_RBLH = [ERD_RBLH bandpower(active.LH,out.fs,[15 30])];
                    ERD_RBRH = [ERD_RBRH bandpower(active.RH,out.fs,[15 30])];
                ...
                times = 1:nchunks;
                times = times .* chunksize;
                times = times - chunksize - 2;
...
fname = ['/Volumes/HDD/data/BCI/work/alldat-' int2str(chunksize*1000) '-trials-time-base.csv'];
fancy_csv(fname,output2);